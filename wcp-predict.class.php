<?php
/**
 * Class that generates prediction forms for the WorldCup plugin.
 * 
 * @version $Id: wcp-predict.class.php 2153552 2019-09-09 12:55:03Z landoweb $
 * @author landoweb
 * @package WorldCup 
...
class WorldCupPredict extends WorldCup {
	 ...
	 * 
	 * @param $stage - Show only matches from a particular stage or group
	 * @param $limit - Limit to n matches
	 * @param $group - Show only matches of the group stage
	 * @param $kickoff - Sort by kickoff time. Mutually exclusive with $stage
	 * @return unknown_type
	 */
	function prediction_form($stage, $limit = 0, $group = false, $kickoff = false, $predict_penalties = false) {
		...
		if (!$logged_in) {
			...
			$this->setMessage(sprintf(__('Please <a href="%1$s">login</a> or <a href="%2$s">register</a> to make a prediction', WCP_TD), $login_url, $register_url),false);
			$disabled = ' disabled ';			
		}
		...
		if (empty($result)) {
			return __('No matches available in this championship.', WCP_TD);
		}		
		
		$save = '<p class="save-button"><input '.$disabled.' class="button" type="submit" value="'.__('Save', WCP_TD).'" name="'.$this->prefix.'save" /></p>';
		...
		foreach ($result as $row) {
			...
			$output .= '<tr class="predict"><td class="l"><span class="team-flag">'.$this->mklink($this->flag($row->home_country), $row->home_url, $this->unclean($row->home_team_name)) . '</span><div class="team-name">' . $this->mklink($this->unclean($row->home_team_name), $row->home_url, $this->unclean($row->home_team_name));
			$output .= '</div></td><td class="cl"><input '.$disabled.' type="text" tabindex="'.$tabindex.'" maxlength="1" value="'.$this->score($match,$row->match_id,'home').'" name="match['.$row->match_id.'][home]" /></td>';
			$tabindex++;
			$output .= "<td class='c'>$separator</td>";
			$output .= '<td class="cr"><input '.$disabled.' type="text" tabindex="'.$tabindex.'" maxlength="1" value="'.$this->score($match,$row->match_id,'away').'" name="match['.$row->match_id.'][away]" /></td><td class="r"><span class="team-flag">'.$this->mklink($this->flag($row->away_country), $row->away_url, $this->unclean($row->away_team_name)).'</span><div class="team-name">';
			$tabindex++;
			$output .= $this->mklink($this->unclean($row->away_team_name), $row->away_url, $this->unclean($row->away_team_name)).'</div></td></tr>' . PHP_EOL;
			
			/**
			 * For knockout stage allow penalty scores.
			 */
			if (!$row->is_group && $predict_penalties) {
				$output .= '<tr><td class="r">' . __('Penalties', WCP_TD);
				$output .= '</td><td class="cl"><input '.$disabled.' type="text" tabindex="'.$tabindex.'" maxlength="1" value="'.$this->score($match,$row->match_id,'pen_home').'" name="match['.$row->match_id.'][pen_home]" /></td>';
				$tabindex++;
				$output .= '<td class="c">x</td>';
				$output .= '<td class="cr"><input '.$disabled.' type="text" tabindex="'.$tabindex.'" maxlength="1" value="'.$this->score($match,$row->match_id,'pen_away').'" name="match['.$row->match_id.'][pen_away]" /></td><td class="l">';
				$tabindex++;
	...
	function ajax() {
		...
		if (isset($_POST['match'])) {
			...
			$this->setMessage(__('No data to save', WCP_TD), true);
		}
		...
		$w = new WorldCupPredictionsWidget();
		$show_total = false;
		$show_result = false;
		...
		$ret = array('notice' => $this->printMessage(false), 'preds' => $r->user_predictions($show_total, $show_result));
	...
	function countdown_clock($stage, $kickoff) {
		 ...
		 */
		if ($deadline) {
			$fmt = get_option($this->prefix.'countdown_format', __('Next prediction deadline in', WCP_TD) . ' %%D%%d, %%H%%h, %%M%%m, %%S%%s');
			$output .= '<div class="wcup_clock" id="wcup_clock_'.$stage.'">Clock</div>';
			$output .= '
		<script type="text/javascript">
			<!--
			var wcup_cd'.$stage.' = new wcup_countdown(\'wcup_cd'.$stage.'\');
			wcup_cd'.$stage.'.Div			= "wcup_clock_'.$stage.'";
			wcup_cd'.$stage.'.TargetDate	= "'.$deadline['target'].'";
			wcup_cd'.$stage.'.ServerDate	= "'.$deadline['now'].'";
			wcup_cd'.$stage.'.DisplayFormat	= "'.$fmt.'";
			wcup_cd'.$stage.'.FinishStr    	= "'.__('Too Late - Prediction deadline passed', WCP_TD).'";
			wcup_cd'.$stage.'.Setup();
			//-->
		</script>
			...
			$output .= '<div class="wcup_clock" id="wcup_clock_'.$stage.'">' .
				__('No matches available in this championship.', WCP_TD) . '</div>';
		}
